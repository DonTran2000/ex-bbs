...
import com.example.ex_bbs.repository.ArticleRepositoryIn;
import com.example.ex_bbs.repository.CommentRepositoryIn;
...
//
//        // JPA
//        List<Article> articleListScreen = articleRepositoryIn.findAll();
//
...
//    }
    ...
    @Autowired
    private ArticleRepositoryIn articleRepositoryIn;

    @Autowired
    private CommentRepositoryIn commentRepositoryIn;

    @GetMapping("")
    public String index(ArticleForm articleForm, CommentForm commentForm, Model model) {
        // JPA
        List<Article> articleListScreen = articleRepositoryIn.findAllByOrderByIdDesc();
    ...
    public String insertArticle(@Validated ArticleForm articleForm, BindingResult result, CommentForm commentForm, Model model) {
        ...
        article.setName(articleForm.getArticleName());
        article.setContent(articleForm.getArticleContent());
        articleRepositoryIn.save(article);
    ...
    public String insertComment(@Validated CommentForm commentForm, BindingResult result,
        ...
        Article article = articleRepositoryIn.findById(Integer.parseInt(articleId)).get();
        comment.setArticle(article);

        commentRepositoryIn.save(comment);
        return "redirect:/";
    }

    @PostMapping("/deleteArticle")
    public String deleteArticle(String articleId) {
        articleRepositoryIn.deleteById(Integer.parseInt(articleId));
        return "redirect:/";
    }
